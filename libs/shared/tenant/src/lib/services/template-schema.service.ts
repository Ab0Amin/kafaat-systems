...
import { TemplateConfig } from '@kafaat-systems/entities';
...
export class TemplateSchemaService {
  ...
  async getTemplateConfigs(): Promise<TableInfo[]> {
    try {
      const configs = await this.dataSource.getRepository(TemplateConfig).find({
        order: { priority: 'ASC' },
      });
      ...
      this.logger.error(`Error getting template configs: ${error.message}`);
      return [];
    }
  }
  ...
  async cloneTemplateToSchema(targetSchema: string): Promise<void> {
    ...
    if (configs.length === 0) {
      this.logger.warn('No template configurations found, using default template schema');
  ...
  private async cloneTable(
    ...
    try {
      ...
      const targetTableExists = await this.checkTableExists(targetSchema, tableName);
      
      if (targetTableExists) {
        this.logger.warn(`Target table ${targetSchema}.${tableName} already exists, skipping structure creation`);
      } else if (copyStructure) {
        // Create table structure in target schema
        this.logger.debug(`Creating table ${targetSchema}.${tableName} from ${sourceSchema}.${tableName}`);
      ...
      if (copyData && (targetTableExists || copyStructure)) {
        this.logger.debug(`Copying data from ${sourceSchema}.${tableName} to ${targetSchema}.${tableName}`);
        
        // Check if target table has data
        const hasData = await this.tableHasData(targetSchema, tableName);
        
        if (hasData) {
          this.logger.warn(`Target table ${targetSchema}.${tableName} already has data, skipping data copy`);
        } else {
      ...
      this.logger.error(`Error cloning table ${sourceSchema}.${tableName} to ${targetSchema}.${tableName}: ${error.message}`);
      throw error;
    }
  }
