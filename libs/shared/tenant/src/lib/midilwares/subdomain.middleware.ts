    ...
    try {
      ...
      if (hostParts.length > 2) {
        ...
        if (tenant) {
          ...
          this.logger.debug(`Tenant found for subdomain ${subdomain}, using schema: ${tenant.schema_name}`);
        } else {
        ...
        this.logger.debug(`Main domain detected, using schema: ${req.schemaName}`);
      }
      
      next();
    } catch (error) {
      this.logger.error(`Error in subdomain middleware: ${error.message}`);
