...
export class SchemaAccessGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (process.env.NODE_ENV === 'development' && process.env.SKIP_SCHEMA_ACCESS_CONTROL === 'true') {
      return true;
    }
    ...
    if (requiredRoles.length > 0 && !requiredRoles.includes(userRole)) {
      this.logger.warn(`Access denied: User with role ${userRole} does not have required roles ${requiredRoles.join(', ')}`);
    ...
    if (userRole === RoleType.ADMIN) {
      ...
      if (requestedSchema && requestedSchema !== schema && requestedSchema !== 'public') {
        this.logger.warn(`Access denied: Admin of schema ${schema} tried to access schema ${requestedSchema}`);
